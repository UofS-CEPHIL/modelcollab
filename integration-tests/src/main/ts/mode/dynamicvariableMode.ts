...
export async function createDynamicVariable(driver: any, xOffset: number , yOffset: number): Promise<string> {
...
export async function verifyFirebaseWithDynamicVariableNumbers(_: any, expectedSize: number, dm?: FirebaseInteractions): Promise<string> {
    if (!dm)
    ...
    for (var i = 0; i < myComponents.length; i ++){
        ...
        if (myComponent.getType() === schema.ComponentType.VARIABLE)
            count++;
    }

    if (count != expectedSize){
        return `Expected ${expectedSize} variable(s) but found ${count}`;
    }
