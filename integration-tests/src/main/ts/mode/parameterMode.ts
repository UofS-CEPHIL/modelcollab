...
export async function createParameter(driver: any, xOffset: number , yOffset: number): Promise<string> {
...
export async function verifyFirebaseWithParameterNumbers(_: any, expectedSize: number, dm?: FirebaseInteractions): Promise<string> {
    if (!dm)
    ...
    for (var i = 0; i < myComponents.length; i ++){
        ...
        if (myComponent.getType() === schema.ComponentType.PARAMETER)
            count++;
    }

    if (count != expectedSize){
        return `Expected ${expectedSize} parameter(s) but found ${count}`;
    }
