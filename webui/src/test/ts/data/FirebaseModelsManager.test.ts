...
import FirebaseModelsManager from "../../../main/ts/data/FirebaseModelsManager";
...
describe("FirebaseModelsManager", () => {

    function makeModelsManager(): FirebaseModelsManager {
        return new FirebaseModelsManager(
            mockDataModel,
            getCurrentData,
            onDataChanged
        );
    ...
    let modelsManager: FirebaseModelsManager = makeModelsManager();
    ...
    beforeEach(async () => {
        jest.clearAllMocks();
        modelsManager = makeModelsManager();
        currentData = FirebaseModelsList.EMPTY;
        getCurrentData.mockImplementation(() => currentData);
        onDataChanged.mockImplementation(d => {
            currentData = d;
            modelsManager.notifyDataUpdated();
        });
        (mockDataModel.subscribeToModelMetadata as jest.Mock)
            ...
            if (t === ModelUserType.OWNER) {
                ...
                test(
                    ...
                    async () => {
                        expect(currentData).not.toBeNull();
                        expect(
                            currentData!.ownedModels.get(ids[0])
                        ).toBeDefined();
                        expect(
                            currentData!
                                .ownedModels
